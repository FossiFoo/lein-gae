...
(defn- copy-web-xml [project-name]
  "Creates a standard web.xml in war/WEB-INF/"
  (spit "war/WEB-INF/web.xml"
        (reduce (fn [str [pat ret]] (str/replace str pat ret))
                (slurp* (.getResourceAsStream (clojure.lang.RT/baseLoader)
                                              "web.xml"))
                {"$DISPLAY-NAME" project-name
                 "$SN"  project-name
                 "$SERVLET-CLASS" (str project-name ".servlet")})))

(defn- copy-appengine-web-xml [appid appversion]
  "Creates a standard appengine-web.xml in war/WEB-INF/"
  (spit "war/WEB-INF/appengine-web.xml"
        (reduce (fn [str [pat ret]] (str/replace str pat ret))
                (slurp* (.getResourceAsStream (clojure.lang.RT/baseLoader)
                                              "appengine-web.xml"))
                {"$APPID" appid
